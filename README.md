# My Flutter Developer Portfolio

Hi! I'm **Femi Fashola**, a Flutter Developer with 4+ years of experience building production-grade mobile applications.

---

## 📱 Featured Projects

### 1. SEKIAPP – Crypto, Giftcard & Bills Payment
**Role:** Lead Flutter Developer | **Duration:** Sep 2023 – Present

**About:**
High-performance crypto and fintech app serving thousands of users with real-time transaction handling and secure payment processing.

**My Contributions:**
- ✅ Led complete UI redesign using MVVM architecture
- ✅ Implemented Riverpod state management for reactive data flow
- ✅ Integrated RESTful APIs for secure transactions
- ✅ Built light/dark theming, biometric authentication
- ✅ Integrated OneSignal push notifications & Intercom support
- ✅ Managed deployment to Google Play Store & Apple App Store

**Technologies:**
`Flutter` • `Dart` • `MVVM` • `Riverpod` • `Firebase` • `REST APIs` • `OneSignal` • `Paystack`

**Live App:**
- 🎮 [Google Play Store](https://play.google.com/store/apps/details?id=com.wiseki.sekiapp&hl=en)
- 🎮 [Apple App Store](https://apps.apple.com/us/app/sekiapp/id1622624126)

**Screenshots:**
<div style="width: 100%; text-align: center;">
  <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap; margin: 20px auto; max-width: 900px;">
    <img src="https://github.com/Gfash436/my_portfolio/raw/Gfash/sekiapp1.webp" width="210" height="420" style="object-fit: cover;" alt="SEKIAPP 1">
    <img src="https://github.com/Gfash436/my_portfolio/raw/Gfash/sekiapp2.webp" width="210" height="420" style="object-fit: cover;" alt="SEKIAPP 2">
    <img src="https://github.com/Gfash436/my_portfolio/raw/Gfash/sekiapp3.webp" width="210" height="420" style="object-fit: cover;" alt="SEKIAPP 3">
  </div>
</div>

### 2. ShopNig – E-Commerce Platform
**Role:** Lead Flutter Developer | **Duration:** Sep 2023 – Present

**About:**
Full-featured e-commerce marketplace with product listings, shopping cart, payments, and order management.

**My Contributions:**
- ✅ Led complete UI/UX redesign for a responsive cross-platform experience
- ✅ Architected MVVM-based app with clean code principles
- ✅ Integrated RESTful APIs for products, authentication, orders & payments
- ✅ Implemented real-time notifications with OneSignal
- ✅ Built biometric security & Intercom customer support
- ✅ Managed app store deployment pipelines

**Technologies:**
`Flutter` • `Dart` • `MVVM` • `Riverpod` • `Firebase` • `REST APIs` • `Stripe`

**Live App:**
- 🎮 [Google Play Store](https://play.google.com/store/apps/details?id=com.wiseki.shopnig&hl=en)

---

### 3. Supply Smart – Agency Banking & POS
**Role:** Mobile App Engineer (Remote) | **Duration:** May 2023 – May 2025

**About:**
Comprehensive banking and point-of-sale application for agency services with account management and bill payments.

**My Contributions:**
- ✅ Built responsive UI for iOS and Android from scratch
- ✅ Integrated authentication flows and virtual account management
- ✅ Implemented bill payment and fund transfer functionality
- ✅ Optimised core business logic for performance
- ✅ Ensured app stability through comprehensive testing

**Technologies:**
`Flutter` • `Dart` • `REST APIs` • `Firebase` • `SQLite`

**Live App:**
- 🎮 [Google Play Store](https://play.google.com/store/apps/details?id=com.supplysmart.supplysmartapp&hl=en)

### 4. Plug.ng – Financial Services
**Role:** Lead Flutter Developer | **Duration:** Sep 2023 – Present

**About:**
A bill payment, virtual card, and gift card services platform with KYC verification.

**My Contributions:**
- ✅ Built application from scratch using Flutter & MVVM architecture
- ✅ Integrated KYC verification for compliance
- ✅ Implemented virtual card services
- ✅ Built bill payment functionality
- ✅ Conducted end-to-end testing & store compliance

**Technologies:**
`Flutter` • `Dart` • `MVVM` • `REST APIs` • `Firebase`

**Live App:**
- 🎮 [Google Play Store](https://play.google.com/store/apps/details?id=com.plugdot.plugng&hl=en)
- 🎮 [Apple App Store](https://apps.apple.com/us/app/plug-ng-data-airtime-cards/id6745476904)

---

## 🛠️ Tech Stack & Skills

**Languages:** Dart, Flutter, Kotlin (Basic), Swift (Basic)

**Architecture:** MVVM, Clean Architecture, OOP, Dependency Injection

**State Management:** Riverpod, Provider, BLoC

**APIs & Backend:** REST, GraphQL (Basic), WebSockets, Firebase

**Databases:** Firebase Firestore, SQLite, Hive

**Tools:** Git, GitHub Actions, Codemagic, Android Studio, Xcode, VS Code

**Payment Gateways:** Stripe, Flutterwave, Paystack

**Testing:** Unit Testing, Widget Testing, Integration Testing

**Soft Skills:** Agile, Remote Collaboration, Code Reviews

---

## 📊 Quick Stats

- 🚀 **4+ production apps** deployed to major app stores
- 📱 **4+ years** professional Flutter development experience
- 🌍 **Remote-ready** developer experienced with distributed teams
- ⚡ **Performance-focused** – optimised apps with 0.3% crash rates
- 🔒 **Security-conscious** – integrated payment gateways and biometric auth

---

## 📫 Get in Touch

- **Portfolio:** [femifasholaportfolio.netlify.app](https://femifasholaportfolio.netlify.app/)
- **LinkedIn:** [linkedin.com/in/femifashola](https://linkedin.com/in/femifashola)
- **Email:** fashola436@gmail.com
- **Phone:** +234 8166782888

---

**Last Updated:** 12th June, 2026
